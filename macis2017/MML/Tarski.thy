...
theory Tarski
  imports "../Mizar/Mizar" "../Mizar/Mizar_Defs" "../Mizar/Mizar_By"
begin
...
where
  ...
  hidden_mode[rule_format,simp,intro,type_infer]: "x be set implies x be object"

abbreviation not_equal (infixl "<>" 50) where
  "x <> y \<equiv> not (x = y)"

lemma not_eq: "x <>y implies y<>x" using not_cong by auto   
...
lemmas tarski[intro!] = tarski_0_2[simplified,rule_format]
...
proof (induct rule: means_property[OF tarski_def_1_def,of y, case_names existence uniqueness])
    case uniqueness
      ...
      thus "X1 = X2" using tarski_th_2[of X1 X2] A1 A2 T0 by auto
 next    
   case existence  
      obtain X where
...
lemmas tarski_def_1a[simp,type_infer] = tarski_def_1[THEN conjunct1,THEN conjunct1,simplified]
...
next
  ...
  thus "IT1 = IT2" using tarski_th_2[of IT1 IT2] A by auto
qed

lemmas tarski_def_2b[simp] = tarski_def_2[THEN conjunct1,THEN conjunct2,simplified,rule_format]
lemmas tarski_def_2a[simp,type_infer] = tarski_def_2[THEN conjunct1,THEN conjunct1,simplified,rule_format]
...
proof (induct rule: means_property[OF tarski_def_4_def,of X,case_names existence uniqueness])
  case existence
...
lemmas tarski_def_4a[simp,type_infer] = tarski_def_4[THEN conjunct1,THEN conjunct1,OF all_set]
...
proof (intro ballI notI impI)
  ...
  obtain Y where
  A4: "Y be set & Y in ?X & not(ex z being object st z in ?X & z in Y)"
    using tarski_0_5[of "?X"] T0 tarski_def_1b by auto
...
abbreviation tarski_def_5_L ("\<langle> _ , _ \<rangle>") where
...
definition sethood :: "Mode \<Rightarrow> o" ("sethood-of _" 190) where  
  "sethood-of M \<equiv>
    ex X being set st for x being object holds x be M iff x in X"  
text_raw \<open>}%EndSnippet\<close> 

abbreviation (input) setS :: "Set \<Rightarrow> Mode" ("set''")where
