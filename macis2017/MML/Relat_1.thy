...
definition Relation_like :: Attr where relat_1_def_1a[THEN defattr_property]:
  "Relation_like \<equiv> define_attr (\<lambda>IT. IT be set & (for x holds x in IT implies (ex y, z st x = [y, z])))"

definition nonRelation_like :: Attr where relat_1_def_1b[THEN defattr_property]:
...
theorem relat_1_cl_1: 
  "cluster empty \<rightarrow> Relation_like for set" using relat_1_def_1a by auto
...
lemmas [simp] = relat_1_def_7[THEN conjunct1,THEN conjunct2,rule_format,simplified]
lemmas [simp,type_infer] = relat_1_def_7 [THEN conjunct1,THEN conjunct1,simplified]
...
theorem relat_1_th_26:
  assumes "P be Relation" "R be Relation"
  shows "rng (P*R) c= rng R"
proof(unfold tarski_def_3,rule ballI, rule impI)
  have T0: "P*R be Relation" using relat_1_def_9 assms by auto
  fix y assume "y in rng (P*R)"
  then obtain x where
     "x be object" "[x,y] in P*R" using xtuple_0_def_12 T0 by auto
  then obtain z where
    "z be object" "[x,z] in P & [z,y] in R" using relat_1_def_9 assms by auto
  thus "y in rng R" using xtuple_0_def_12 assms by auto
qed  
...
lemmas [simp,type_infer] = relat_1_def_8 [THEN conjunct1,THEN conjunct1,simplified,rule_format,OF all_set]
...
mtheorem relat_1_th75:  
 "X \<subseteq> Y implies R|X \<subseteq> R|Y"
proof
  assume A0:"X \<subseteq> Y"
  show "R|X \<subseteq> R|Y"
  proof(unfold tarski_def_3,rule ballI,rule impI)
    fix x 
    have A1:"R|X be Relation" using assms relat_1_def_11[of R X,THEN conjunct1] by auto
    assume A2: "x in R|X"
    then obtain a b where
       A3:"a be object" "b be object" "x=[a,b]" using A1 relat_1_def_1a by auto  
    have "a in X & [a,b] in R" using relat_1_def_11[OF assms(3,1), THEN conjunct1] assms A2 A3 by auto
    hence "a in Y & [a,b] in R" using A0 by auto
    thus "x in R|Y" using relat_1_def_11[OF assms(3,2), THEN conjunct1] assms A2 A3 by auto
  qed
qed  
...
theorem relat_1_cl_20:
   "let X be empty \<parallel> set 
    cluster dom X \<rightarrow> empty" using relat_1_def_1a by auto
