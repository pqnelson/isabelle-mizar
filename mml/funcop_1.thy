...
func funcop_1_def_2 (infix "-->" 90) where
...
proof-
  ...
  thus "(X -->y) is Function_like \<bar> Relation_like" using A funcop_1_def_2[of X y] by auto
qed

reserve A for set

mtheorem funcop_1_th_7:
  "x in A \<longrightarrow> (A --> y).x = y"
proof
  have Z:"y in {y}" using tarski_def_1 by auto
  have T0:"(A --> y) be Function" by auto
  assume "x in A"
...
thm ty
  ...
  proof(intro tarski_def_3b)
    ...
    thus "x in dom (A -->y)" using W3 xtuple_0_def_12 by auto
  qed simp_all
  thus "dom (A -->y) = A" using A1 xboole_0_def_10 W3 by auto
  show "rng (A -->y) c= {y}" using W3 relat_1_def_19[THEN iffD1,simplified,rule_format] by mauto
qed

func funcop_1_def_9 (infix ".-->" 300) where
...
mtheorem funcop_1_cl[ty_func_cluster]:
  mlet "x be object", "y be object"
   "cluster x .--> y \<rightarrow> Function_like \<bar>Relation_like"
proof-
  have "x .--> y = {x}--> y" using funcop_1_def_9 by auto
  thus "x .--> y is Function_like \<bar>Relation_like" by mauto
qed
