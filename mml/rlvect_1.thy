...
attr rlvect_1_def_3 ("add-associative")
...
proof(simp,intro conjI)
  ...
  proof(intro rlvect_1_def_3I ballI)
    ...
    show "(u \<oplus>\<^sub>?T v) \<oplus>\<^sub>?T w = u \<oplus>\<^sub>?T (v \<oplus>\<^sub>?T w)" by (intro struct_0_def_10a[of ?T]) mauto
  qed simp_all
  show "?T be right-zeroed"
  proof(intro rlvect_1_def_4I ballI)
    fix u v w assume [ty]: "v be Element-of-struct ?T"
    show "v \<oplus>\<^sub>?T 0\<^sub>?T = v" by (intro struct_0_def_10a[of ?T]) mauto
  qed simp_all
qed

theorem rlvect_cl_2[ex]:
...
proof(intro ballI impI)
  ...
  obtain u where
    ...
    A2: "w \<oplus>\<^sub>V u = 0\<^sub>V" using algstr_0_def_11E by mauto
...
proof(intro algstr_0_def_7I ballI)
  ...
  obtain v1 where
    ...
    A1: "v \<oplus>\<^sub>V v1 = 0\<^sub>V" using algstr_0_def_11E by mauto
  show "v is right-add-cancelable\<^sub>V"
  proof(intro algstr_0_def_4I)
...
proof(intro ballI conjI)
  ...
  obtain w where
    ...
    A1: "v \<oplus>\<^sub>V w = 0\<^sub>V" using algstr_0_def_16E[of V v] algstr_0_def_11E[of V v] by auto
  ...
  thus "0\<^sub>V \<oplus>\<^sub>V v = v" using rlvect_1_def_3E[of V v w v] A1 A2 by simp
qed auto

theorem rlvect_1_reduce_1[simplified,rule_format]:
  "let V be non empty-struct\<bar>right-complementable\<bar>add-associative\<bar>right-zeroed\<bar> addLoopStr&
      v1 be zero \<^sub>V \<bar>Element-of-struct V \<and>
      v2 be Element-of-struct V
   reduce v1 \<oplus>\<^sub>V v2 to v2"
proof(elim conjE)
  assume T0[ty]: "V be non empty-struct\<bar>right-complementable\<bar>add-associative\<bar>right-zeroed\<bar> addLoopStr""
...
proof(elim conjE)
  assume T0[ty]: "V be non empty-struct\<bar>right-complementable\<bar>add-associative\<bar>right-zeroed\<bar>
           addLoopStr"
      "v1 be zero \<^sub>V \<bar>Element-of-struct V"
      "v2 be Element-of-struct V"
  have "v1 = 0\<^sub>V" using struct_0_def_12E[of V v1] by auto
...
theorem rlvect_1_def_10[rule_format]:
  "let V be non empty-struct \<bar>addLoopStr&
       v be Element-of-struct V \<and>
...
proof(elim conjE,rule redefine_func_means_property,simp)
  assume [ty]:"V be non empty-struct \<bar>addLoopStr"
        "V is add-associative\<bar>right-zeroed\<bar>right-complementable"
         "v be Element-of-struct V"


  thus T: "(\<ominus>\<^sub>V v) be Element-of-struct V" by mauto
  fix IT assume [ty]:"IT be Element-of-struct V"

  show "IT = \<ominus>\<^sub>V v \<longleftrightarrow> v \<oplus>\<^sub>V IT = 0\<^sub>V"
  proof(rule iffI3)
    obtain v1 where
      T1[ty]:"v1 be Element-of-struct V" and
      A2: "v \<oplus>\<^sub>V v1 = 0\<^sub>V" using algstr_0_def_16E[of V v] algstr_0_def_11E[of V v]  by auto
    ...
    proof(intro algstr_0_def_10I)
      ...
      proof(rule bexI[of _ _ v1])
        ...
        have "(v1 \<oplus>\<^sub>V v) \<oplus>\<^sub>V v1 = v1 \<oplus>\<^sub>V 0\<^sub>V" using rlvect_1_def_3E[of V v1 v v1] A2 by simp
        ...
        thus "v1 \<oplus>\<^sub>V v = 0\<^sub>V" using algstr_0_def_7E[of V v1] algstr_0_def_4E[of V v1 "v1 \<oplus>\<^sub>V v" "0\<^sub>V"] A3 by mauto
      qed simp_all
    qed simp_all
    have "v \<oplus>\<^sub>V \<ominus>\<^sub>V v \<oplus>\<^sub>V v = v \<oplus>\<^sub>V (\<ominus>\<^sub>V v \<oplus>\<^sub>V v)" using rlvect_1_def_3E[of V v "\<ominus>\<^sub>V v" v]  A2 by mauto
    also have "\<dots> = v \<oplus>\<^sub>V 0\<^sub>V" using algstr_0_def_7E[of V v] algstr_0_def_13[of V v]  A4 by mty auto
    also have "\<dots> = 0\<^sub>V \<oplus>\<^sub>V v" using rlvect_1_reduce_1[of V "0\<^sub>V" v] rlvect_1_reduce_2[of V "0\<^sub>V" v] by mauto
    finally show "IT =\<ominus>\<^sub>V v \<longrightarrow> v \<oplus>\<^sub>V IT = 0\<^sub>V" using algstr_0_def_4E[of V v "v \<oplus>\<^sub>V \<ominus>\<^sub>V v" "0\<^sub>V"] algstr_0_def_7E[of V v] by mauto
    assume A5: "v \<oplus>\<^sub>V IT = 0\<^sub>V"
    have "IT = 0\<^sub>V \<oplus>\<^sub>V IT" using rlvect_1_reduce_1[of V "0\<^sub>V" IT] by mty auto
    also have "\<dots> = \<ominus>\<^sub>V v \<oplus>\<^sub>V v \<oplus>\<^sub>V IT" using algstr_0_def_7E[ of V v] algstr_0_def_13[of V v] A4 by mauto
    also have "\<dots> = \<ominus>\<^sub>V v \<oplus>\<^sub>V 0\<^sub>V" using rlvect_1_def_3E[of V "\<ominus>\<^sub>V v" v IT] A2 A5 by mauto
...
proof(intro ballI impI)
  ...
  thus "v = \<ominus>\<^sub>V w" using T0 by (auto intro: rlvect_1_def_10[THEN conjunct2,THEN conjunct2,simplified,rule_format])
qed auto
...
proof(intro ballI)
  ...
  show "ex w be Element-of-struct V st v \<oplus>\<^sub>V w = u"
    proof(rule bexI[of _ _ "\<ominus>\<^sub>V v \<oplus>\<^sub>V u"])
...
proof(intro ballI impI)
  ...
  proof (cases "v1 \<oplus>\<^sub>V w = v2 \<oplus>\<^sub>V w")
    ...
    next
      ...
      also have "\<dots> =  \<ominus>\<^sub>V w \<oplus>\<^sub>V ( w \<oplus>\<^sub>V v1)" using rlvect_1_def_3E[of V "\<ominus>\<^sub>V w" w v1 ] by mauto
      also have "\<dots> =  \<ominus>\<^sub>V w \<oplus>\<^sub>V w \<oplus>\<^sub>V v2" using A1 Z rlvect_1_def_3E[of V "\<ominus>\<^sub>V w" w v2] by mauto
