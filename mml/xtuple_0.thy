...
proof (cases "y1=y2")
  ...
  hence "y1 = x1 \<and> y1 = x2" using B xtuple_0_lm_1[of "y1" "x1" "x2"] xtuple_0_lm_1[of "y1"] by mby auto
...
proof -
    assume [ty]: "x is pair"
    obtain x1 x2 where
       [ty]:"x1 be object" "x2 be object" and A1: "x = [x1,x2]" using xtuple_0_def_1[of x] by auto
    show "ex it being object st for y1,y2 be object st x=[y1,y2] holds it = y1"
      proof(rule bexI[of _ _ x1],simp,intro ballI impI)
...
func xtuple_0_def_3 ("_ `2" [90] 95) where
 mlet "x be object"
...
proof -
  ...
  obtain x1 x2 where
    ...
    show "ex it being object st for y1,y2 be object st x=[y1,y2] holds it = y2"
      proof(rule bexI[of _ _ x2],simp,intro ballI impI)
...
proof
  have B: "[x,y] be set " using tarski_def_5[of y x] by mauto
  assume A1: "[x,y] in X"
  have A0: "{{x,y},{x}} =[x,y]" using tarski_def_5 by auto
  hence "{x,y} in [x,y]" using tarski_def_2[of "{x,y}" "{x}"] by mauto
  hence "ex Y being set st ({x,y} in Y \<and> Y in X)" using A1 B bexI[of _ _ "[x,y]"] by auto
  hence A2: "{x,y} in union X" using tarski_def_4 by auto
  have "y in {x,y}" using tarski_def_2 by simp
  hence "ex Y being set st y in Y \<and> Y in (union X)" using A2 exI[of _ "{x,y}"] by mauto
  thus "y in union union X" using tarski_def_4 by auto
qed

func xtuple_0_def_12 ("proj1 _" [95] 95) where
  mlet "X be set"
...
func xtuple_0_def_13 ("proj2 _" [95] 95) where
   mlet "X be set"
...
mtheorem xtuple_0_th_8:
 "X \<subseteq> Y \<longrightarrow> proj1 X \<subseteq> proj1 Y" using tarski_def_3 xtuple_0_def_12 by auto

mtheorem xtuple_0_th_9:
 "X \<subseteq> Y \<longrightarrow> proj2 X \<subseteq> proj2 Y" using tarski_def_3 xtuple_0_def_13 by auto


mtheorem xtuple_th_23:
 "proj1 (X \<union> Y) = (proj1 X) \<union> (proj1 Y)"
proof(rule xboole_0_def_10a,simp+)
  show "proj1 (X \<union> Y) c= (proj1 X) \<union> (proj1 Y)"
  proof(rule tarski_def_3b)
    fix x
    assume "x in proj1 (X\<union>Y)"
    then obtain y where
       "y be object" and A1:"[x,y] in X\<union>Y" using xtuple_0_def_12 by auto
    have "x in proj1 X \<or> x in proj1 Y" using xboole_0_def_3 A1 xtuple_0_def_12 by auto
    thus "x in (proj1 X) \<union> (proj1 Y)" using xboole_0_def_3 by auto
  qed simp_all
  show "(proj1 X) \<union> (proj1 Y) c= proj1 (X \<union> Y)"
  proof(rule tarski_def_3b)
    fix x
    assume "x in (proj1 X) \<union> (proj1 Y)"
    hence A: "x in proj1 X \<or> x in proj1 Y" using xboole_0_def_3 by auto
     have "X c= X\<union>Y \<and> Y c= X\<union>Y" using tarski_def_3 xboole_0_def_3 by auto
     hence "proj1 X c= proj1 (X\<union>Y) \<and> proj1 Y c= proj1 (X\<union>Y)"
          using xtuple_0_th_8[of _ X] using xtuple_0_th_8[of _ Y] by auto
     thus "x in proj1 (X \<union> Y)" using A tarski_def_3 by auto
  qed simp_all
qed
...
proof(rule xboole_0_def_10a,simp+)
  ...
  proof(rule tarski_def_3b)
    ...
    then obtain y where
       "y be object" and A1:"[y,x] in X\<union>Y" using xtuple_0_def_13 by auto
    have "x in proj2 X \<or> x in proj2 Y" using xboole_0_def_3 A1 xtuple_0_def_13 by auto
    thus "x in (proj2 X) \<union> (proj2 Y)" using xboole_0_def_3 by auto
  qed simp_all
  show "(proj2 X) \<union> (proj2 Y) c= proj2 (X \<union> Y)"
  proof(rule tarski_def_3b)
    fix x
    assume "x in (proj2 X) \<union> (proj2 Y)"
    hence A: "x in proj2 X \<or> x in proj2 Y" using xboole_0_def_3 by auto
     have "X c= X\<union>Y \<and> Y c= X\<union>Y" using tarski_def_3 xboole_0_def_3 by auto
     hence "proj2 X c= proj2 (X\<union>Y) \<and> proj2 Y c= proj2 (X\<union>Y)" using xtuple_0_th_9 by auto
     thus "x in proj2(X \<union> Y)" using A tarski_def_3 by auto
  qed simp_all
qed
