...
theorem Fraenkel_sch_1:
  assumes T0[ty]:"A be set" and
          T1:  "\<forall>x : object.  F(x) be object"
...
theorem Fraenkel_sch_3:
  assumes T0[ty]:"B be set" and
          TT:   "\<forall>x : object.  F(x) be object"
...
theorem Fraenkel_sch_4:
  assumes [ty]:"A be set" "B be set"
      and T0: "for x1 be object, x2 be object holds F(x1,x2) be object"
...
theorem Fraenkel_sch_5:
  assumes [ty]:"B be set"
     and T0: "\<forall>x : object.  F(x) be object"
             "\<forall>x : object.  G(x) be object"
...
proof(intro ballI impI)
  ...
  assume A2:"x in X"
  hence "F. x = (G|X).x" using A1 funct_1_th_49[of F X x]  by simp
  also have "... =  G. x" using A2 funct_1_th_49[of G X x] by simp
...
proof(rule ballI, rule ballI, rule impI)
  fix s t
  assume T0[ty]: "s be Element-of A" "t be Element-of B"
  assume "P(s,t)"
  then have "F(s,t) in { F(s1,t1) where s1 be Element-of A,t1 be Element-of B:P(s1,t1)}" using Fraenkel_A2_in by auto
