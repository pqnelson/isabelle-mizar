...
proof(rule ballI,rule impI)
  fix B assume T0[ty]:"G be Group" "A be (Subset-of-struct G)" "B be (Subset-of-struct G)" and
  ...
  have S: "sethood(Element-of-struct G)" by simp
  ...
  proof (unfold xboole_0_def_10[OF all_set all_set],rule conjI)
    ...
    proof(intro tarski_def_3b)
      ...
      have C1: "the carrier of G is non empty" by auto
      have T2[ty]: "a be (Element-of-struct G)" using A1 T1 tarski_def_3a[of A "the carrier of G"] Element(6) by auto
    ...
    proof(intro tarski_def_3b)
      ...
      obtain b where
         [ty]: "b be (Element-of-struct G)" and A4:"a = b\<^sup>-\<^sup>1\<^sub>G" " b in B" using group_2_def_1 Fraenkel_A1_ex[OF _ _ A3] by auto
      hence A5: "b in {g\<^sup>-\<^sup>1\<^sub>G where g be Element-of-struct G:g in A}" using A0 group_2_def_1[of G A] by auto
      obtain c where
         [ty]: "c be (Element-of-struct G)" and "b = c\<^sup>-\<^sup>1\<^sub>G" " c in A" using group_2_def_1 Fraenkel_A1_ex[OF _ _ A5] by auto
...
proof(intro ballI,rule iffI3)
  ...
  show "x in A \<Otimes>\<^sub>G B \<longrightarrow> (ex a,b be Element-of-struct G st x = a \<otimes>\<^sub>G b \<and> a in A \<and> b in B)"
    using Fraenkel_A2_ex group_2_def_2 by auto
