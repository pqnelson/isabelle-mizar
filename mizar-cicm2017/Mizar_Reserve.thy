...
("", 50))]) *) lthy
  ...
  val _ = if fastype_of tm <> @{typ "Mode"}
          then raise Fail "not a correctly typed constraint"
           else ();
  fun reserve thy =
      fold (fn v => fn thy => Miz_Reserve_Data.map (Symtab.update (v, tm)) thy)
        vs thy;
in
  Local_Theory.background_theory reserve lthy
end;
...
fun setup_proof ((name, attr), str) lthy =
  let
    ...
    val _ = if filter_out (fn (_, ty) => ty = @{typ Set}) fs <> []
