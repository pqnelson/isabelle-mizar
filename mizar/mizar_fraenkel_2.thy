...
schematic_goal Fraenkel_A2:
  ...
  assumes [ex]:"inhabited(L1)" "inhabited(L2)" "sethood(L1)" "sethood(L2)" shows "?X"
   ...
   case existence
      ...
      [ty]:"IT be set" and A5: "for x being object holds (x in IT iff
       ...
       show "ex IT being set st (for x being object holds (x in IT \<longleftrightarrow> (ex y1 being L1,y2 being L2 st (x = P (y1,y2)  \<and> Q(y1,y2)))))"
         proof (intro bexI[of _ _ "IT"] ballI)
              ...
              proof (intro iffI2)
                 ...
                 show " (ex y1 being L1,y2 being L2 st x = P(y1,y2)  \<and> Q(y1,y2)) \<longrightarrow> x in IT"
                    proof
                      ...
                      hence A13: "[y1,y2] in [:X1,X2:]" using zfmisc_1_def_2 by auto
...
qed simp_all
  ...
  lemma [ty_func]:  "Fraenkel2(P, L1,L2, Q)  be set" using all_set by auto



lemmas Fraenkel_A2_ex =  Fraenkel_A2[THEN conjunct2,THEN conjunct1,THEN bspec,simplified,THEN iffD1]

theorem Fraenkel_A2_in:
   "sethood(M1) \<Longrightarrow> sethood(M2) \<Longrightarrow> x1 be M1\<Longrightarrow> x2 be M2 \<Longrightarrow>
     Q(x1,x2) \<Longrightarrow> P(x1,x2) in { P(x1,x2) where x1 be M1, x2 be M2 : Q(x1,x2)}"
proof-
  assume A:"sethood(M1)" "sethood(M2)" "x1 be M1" "x2 be M2" "x1 be M1" "Q(x1,x2)"
