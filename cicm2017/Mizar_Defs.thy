...
proof
  ...
  have "(theM ?P) be define_mode(?P)" using the_property[OF] e by auto
...
shows "F be M \<and> newCondition(F)"
...
where "let lt redefine func F \<rightarrow> M means newCondition  \<equiv> ( lt \<Longrightarrow> F be M \<and> newCondition (F))"
text_raw \<open>}%EndSnippet\<close>
...
where "let lt pred def means exp \<equiv> (\<lambda>it. (lt ==>  exp(it)))"  
...
abbreviation (input) assume_mode_prefix
   ("assume _ mode _ \<rightarrow> _ means _" [0,0,0] 10)
where "assume as mode def \<rightarrow> type means condition \<equiv>
   def = theM (\<lambda>it.(it be type & (as implies condition (it))))"
text_raw \<open>}%EndSnippet\<close>
